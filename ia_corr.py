...
class compute_fast_wgp(project_corr):
    ...
    def execute(self,new_shapes=False,shapes=[]):
        """
        Compute the galaxy-shear correlation.

        Arguments
        ----------
        new_shapes : bool, optional
            If True, compute using a new shape catalog provided in `shapes`. Default is False.
        shapes : list, optional
            If new_shapes is True, hosts the catalogue of the shapes (pandas array).
        Attributes:
        ----------
        rp (ndarray): Array of projected radial separation bins.
        mean_logrp (ndarray): Mean logarithmic projected radial separation bins.
